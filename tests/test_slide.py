...
def simple_page() -> Component:
    ...
    return cast(Component, {
        ...
        'props': {},
        'children': [
            {'tag': 'text', 'props': {'content': 'Hello'}, 'children': []}
        ]
    })
...
def test_add_multiple_pages(empty_slide: Slide) -> None:
    ...
    page1 = cast(Component, {'tag': 'page', 'props': {}, 'children': []})
    page2 = cast(Component, {'tag': 'page', 'props': {}, 'children': []})
...
def test_build_applies_transforms(empty_slide: Slide) -> None:
    ...
    page = cast(Component, {
        ...
        'props': {},
        'children': [
            {'tag': 'text', 'props': {'count': 0}, 'children': []}
        ]
    })
...
def test_export_applies_html_transforms(empty_slide: Slide, tmp_path: Path) -> None:
    ...
    page = cast(Component, {'tag': 'page', 'props': {}, 'children': []})
    empty_slide.add_page(page)
...
def test_export_sets_context_page_numbers(empty_slide: Slide, tmp_path: Path) -> None:
    ...
    # Add three pages
    for _ in range(3):
        empty_slide.add_page(cast(Component, {'tag': 'page', 'props': {}, 'children': []}))
...
def test_full_slide_workflow(tmp_path: Path) -> None:
    ...
    # Add pages
    page1 = cast(Component, {
        'tag': 'page',
        'props': {},
        'children': [
            {'tag': 'header', 'props': {'text': 'First Page', 'level': 1}, 'children': []},
            {'tag': 'text', 'props': {'content': 'Content of first page'}, 'children': []}
        ]
    })

    page2 = cast(Component, {
        'tag': 'page',
        'props': {},
        'children': [
            {'tag': 'header', 'props': {'text': 'Second Page', 'level': 1}, 'children': []},
            {'tag': 'text', 'props': {'content': 'Content of second page'}, 'children': []}
        ]
    })
...
def test_multiple_plugins_work_together(tmp_path: Path) -> None:
    ...
    page = cast(Component, {
        ...
        'props': {},
        'children': [
            {'tag': 'header', 'props': {'text': 'Title'}, 'children': []},
            {'tag': 'text', 'props': {'content': 'Body'}, 'children': []}
        ]
    })
